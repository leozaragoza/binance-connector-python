...
@mock_http_response(responses.GET, "/fapi/v1/order\\?symbol=&orderId=", mock_exception, 400)
...
@mock_http_response(responses.GET, "/fapi/v1/order\\?symbol=BTCUSDT&orderId=", mock_exception, 400)
def test_get_order_without_order_id():
    """Tests the API endpoint to get order without provide order id"""

    client = Client(key, secret)
    client.get_order.when.called_with("BTCUSDT", orderId="").should.throw(ClientError)


@mock_http_response(
    responses.GET, "/fapi/v1/order\\?" + urlencode(params), mock_item, 200
)
def test_get_order():
